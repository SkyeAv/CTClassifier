...
def _load_table(p: Path, table: str) -> pl.DataFrame:
  df: pl.DataFrame = pl.read_csv(p, has_header=True, separator="|")
...
def _col_lists(df: pl.DataFrame, threshold: int) -> tuple[list[str]]:
  ...
  embeddings: list[str] = [col for col, n in _cardinality(df, string) if n > threshold]
...
def _remove_device_trials(df: pl.DataFrame) -> tuple[pl.DataFrame, str]:
  df = df.filter(pl.col("studies__is_fda_regulated_device") != "t")
  return df.filter(pl.col("studies__is_unapproved_device") != "t")

def dataset(**kwargs) -> tuple[pl.DataFrame, str]:
  dataset_hash: str = gen_hash(**kwargs)
  ...
  if store.is_file():
    ...
    dataset: pl.DataFrame = _read_snapshot(kwargs["archive"], kwargs["tables"])
    ...
    numeric, embeddings, hot_one = _col_lists(dataset, kwargs["threshold"])
    dataset = _z_scores(dataset, numeric)
    dataset = _hot_one_encode(dataset, hot_one)
    dataset = _embed_texts(dataset, embeddings, kwargs["model"], dataset_hash)
...
def labelset(df: pl.DataFrame, labels: Path, test_size: float, seed: int) -> tuple[lgb.Dataset, float, str]:
  ...
  X: npt.NDArray[np.float16] = labelset.select(pl.exclude(exclude)).to_numpy().astype(np.float16)
  ...
  return dtrain, dtest, scale_pos_weight, label_hash

def prodset(df: pl.DataFrame) -> tuple[npt.NDArray[np.float16], npt.NDArray[np.bytes_]]:
